...
!   USE ldfdyn         ! lateral diffusion: eddy viscosity coef.
   ...
   !                                    != Field splitting =!
   LOGICAL , PUBLIC ::   ln_ldftra_split = .FALSE.      !: flag for field splitting procedure
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:)     ::   vslp, wslpj          !: j-slope at V- and W-pointsints
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:)     ::   wslpi_l              !: i-slope at W-points
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:)     ::   wslpj_l              !: j-slope at W-points
...
!!gm end modif
   ...
   SUBROUTINE ldf_slp_init
      ...
      ELSE                             ! Madec operator : slopes at u-, v-, and w-points
         ...
         IF( ln_ldftra_split ) THEN    ! splitting approach: define two auxillary wslp variables
            IF(lwp) WRITE(numout,*) '              allocated auxiliary arrays for splitting'
            ALLOCATE( wslpi_l(jpi,jpj,jpk), wslpj_l(jpi,jpj,jpk) , STAT=ierr )
            IF( ierr > 0 )   CALL ctl_stop( 'STOP', 'ldf_slp_init : unable to allocate splitting auxiliary variables ' )
            wslpi_l(:,:,:) = 0._wp   ;   wslpj_l(:,:,:) = 0._wp
         ENDIF
