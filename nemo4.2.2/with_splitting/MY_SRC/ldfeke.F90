...
CONTAINS
   ...
   SUBROUTINE ldf_eke( kt, Kbb, Kmm, Kaa )
      ...
      DO_3D( 0, 0, 0, 0, 2, jpkm1 )
         ...
         !
         IF ( ln_ldftra_split ) THEN                                               ! wslpi_l not masked, but zaei[uv]_w is
            zn_slp2 = (   zaeiu_w * wslpi_l(ji,jj,jk) * wslpi_l(ji,jj,jk)   &      ! (slope ** 2) * aeiv
               &        + zaeiv_w * wslpj_l(ji,jj,jk) * wslpj_l(ji,jj,jk)   )
         ELSE
         ...
         IF ( ln_ldftra_split ) THEN   
            zwslpi  = wslpi_l(ji,jj,jk) / MAX( zck, 0.1_wp)                   ! (just to avoid dividing by zeros)
            zwslpj  = wslpj_l(ji,jj,jk) / MAX( zck, 0.1_wp)
         ELSE
            zwslpi  = wslpi(ji,jj,jk) / MAX( zck, 0.1_wp)
      ...
      zeke_ht(:,:) = eke_geom(:,:,Kbb) / MAX( ht(:,:), 1._wp ) * tmask(:,:,1)
      !
     ...
     IF( TRIM(cdrw) == 'READ' ) THEN        ! Read/initialise
        ...
        IF( ln_rstart ) THEN                   !* Read the restart file
           ...
           ELSE                                                 ! one at least array is missing
              ...
              eke_geom(:,:,Kbb)  = rn_eke_init * ht(:,:) * ssmask(:,:)
