...
CONTAINS
   ...
   SUBROUTINE ldf_tra_init
      ...
      NAMELIST/namtra_ldf/ ln_traldf_OFF, ln_traldf_lap  , ln_traldf_blp  ,   &   ! type of operator
         ...
         &                 ln_ldftra_split                                        ! field splitting flag
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_ldf in configuration namelist' )
      ...
      IF(lwp) THEN                      ! control print
         ...
         WRITE(numout,*) '      field splitting                            ln_ldftra_split = ', ln_ldftra_split
      ENDIF
      !
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_eiv in configuration namelist' )
      ...
      ELSE                          !== use the parametrization  ==!
         ...
         !
         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         !!! jm 23 Apr 24: switched off the force stop to have option of GM + no Redi + bilaplacian tracer diffusion
         IF( ln_traldf_blp ) THEN  ! CALL ctl_stop( 'ldf_eiv_init: eddy induced velocity ONLY with laplacian diffusivity' )
            IF(lwp) WRITE(numout,*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
   ...
   SUBROUTINE ldf_eiv_trp( kt, kit000, pu, pv, pw, cdtype, Kmm, Krhs )
      ...
      DO_3D( nn_hls, nn_hls-1, nn_hls, nn_hls-1, 2, jpkm1 )
         IF ( ln_ldftra_split ) THEN
            zpsi_uw(ji,jj,jk) = - r1_4 * e2u(ji,jj) * ( wslpi_l(ji,jj,jk  ) + wslpi_l(ji+1,jj,jk) )   &
               &                                    * ( aeiu   (ji,jj,jk-1) + aeiu   (ji  ,jj,jk) ) * wumask(ji,jj,jk)
            zpsi_vw(ji,jj,jk) = - r1_4 * e1v(ji,jj) * ( wslpj_l(ji,jj,jk  ) + wslpj_l(ji,jj+1,jk) )   &
      ...
      END_3D
!!!!!!!!!!!!!!!!!!!!!!!!!
!! jm (20 Apr 24): only need this here for diagnostic reasons, remove later
      CALL iom_put( "wslpi", wslpi(:,:,:) )
      CALL iom_put( "wslpj", wslpj(:,:,:) )
      CALL iom_put( "wslpi_large", wslpi_l(:,:,:) )
      CALL iom_put( "wslpj_large", wslpj_l(:,:,:) )
!!!!!!!!!!!!!!!!!!!!!!!!!
