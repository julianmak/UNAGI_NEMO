...
!!!    step.F90 is modified so that avt is multiplied by avtb_2d
      IF( ln_zdfcst ) THEN
         IF( nn_havtb == 1 ) THEN
            WRITE(numout,*) '   '
            WRITE(numout,*) '   ==>>>   JM hack: imposing a horizontal structure on constant vertical tracer diffusion'
            WRITE(numout,*) '   ==>>>            amplfication of rn_avt0 = ', rn_avt0, ' over sponge region by factor of rn_avt_amp = ', rn_avt_amp
         END IF
      ...
      END IF
!!!
!!! JM: this fixes avt_k, which is then used repeatedly in zdf_phy
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !
